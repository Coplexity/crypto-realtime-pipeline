"""
OHLC 1h Aggregator - Spark Batch Job
Lấy nến 5m từ MongoDB gom thành nến 1h bằng sức mạnh của PySpark
"""
import os
import sys

# ĐÁNH BAY LỖI WINDOWS: Ép Spark sử dụng đúng Python của môi trường ảo
...
# CẤU HÌNH LOCAL (Đã bỏ pass và trỏ về localhost cho giống hệ thống hiện tại của bạn)
...
def main():
    print("="*70)
    print("🚀 Khởi động luồng Spark: Gom nến 5m -> 1h")
    print("="*70)

    # 1. Khởi tạo trạm điều khiển Spark (Thêm cấu hình Timezone chuẩn UTC)
    spark = SparkSession.builder \
        .appName("OHLC-1h-Aggregator") \
        .config("spark.sql.session.timeZone", "UTC") \
        .getOrCreate()
    
    # 2. Kết nối MongoDB & Đảm bảo Index
    ...
    print("  Đang kiểm tra Index cho MongoDB (1h_kline)...")
    for idx in existing_indexes:
        ...
        if (
            ...
            index_exists = True
            print(f"  ℹ️ Index 1h_kline đã tồn tại: {idx.get('name', 'unnamed')}")
            break
    ...
    if not index_exists:
        try:
            ...
            print("  ✅ Tạo Index 1h_kline thành công!")
        except OperationFailure as e:
            if e.code == 85:
                print("  ℹ️ Index 1h_kline đã tồn tại với tên khác, bỏ qua việc tạo mới.")
            else:
                print(f"  ⚠️ Lỗi tạo Index MongoDB (code {e.code}): {e}")
        except Exception as e:
            print(f"  ⚠️ Lỗi không xác định khi tạo Index: {e}")
    
    # 3. Tính toán khung thời gian (Lùi lại 1 tiếng trọn vẹn)
    ...
    # Lấy danh sách các đồng coin đang có dữ liệu trong giờ này
    symbols_in_db = source_collection.distinct("symbol", {"openTime": {"$gte": start_timestamp, "$lt": end_timestamp}})
    
    if not symbols_in_db:
        print("  ⚠️ Không tìm thấy nến 5m nào trong khung giờ này.")
        spark.stop()
        mongo_client.close()
        return

    # 4. Kéo dữ liệu BSON từ Mongo lên
    query = {
        "symbol": {"$in": symbols_in_db},
        "interval" : "5m",
        "openTime": {"$gte": start_timestamp, "$lt": end_timestamp}
    }
    klines = list(source_collection.find(query).sort("openTime", 1)) # Ép sort để hàm first/last của Spark chạy đúng
    
    print(f"  📥 Lấy được {len(klines)} nến 5m từ MongoDB. Bắt đầu đẩy vào Spark...")
    ...
    for kline in klines:
        # Bỏ qua nếu dòng dữ liệu bị hỏng
        if not isinstance(kline, dict):
            continue
            
        nk = dict(kline)
        # Xóa ID của Mongo vì Spark không hiểu được
        nk.pop("_id", None)
        
        # Xử lý số nguyên (Int64)
        for key in ["openTime", "closeTime", "trades"]:
            if key in nk and isinstance(nk[key], Int64):
                nk[key] = int(nk[key])
                
        # Xử lý số thập phân (Decimal128 & Float)
    ...
    # 6. Đưa vào Spark DataFrame và ép kiểu cột
    ...
    # 7. Sức mạnh của Spark: Gom nhóm theo Symbol và Time
    df = df.withColumn("window_start", (col("openTime") / 3600000).cast("long") * 3600000)
    ...
    # 8. Lưu kết quả lại vào MongoDB
    total_inserted = 0
    total_skipped = 0
    
    for row in results:
        # KIỂM TRA TRƯỚC KHI GHI (Read-before-write)
        ...
        if existing:
            print(f"  ⏭️ 1h OHLC already exists for {row['symbol']} (openTime: {row['openTime']})")
            total_skipped += 1
            continue
        ...
        try:
            # Dùng Upsert theo đúng logic gốc của team bạn
            target_collection.update_one(
                {"symbol": mongo_doc["symbol"], "interval": mongo_doc["interval"], "openTime": mongo_doc["openTime"]},
                {"$set": mongo_doc},
                upsert=True
            )
            print(f"  ✅ {row['symbol']}: 1h OHLC saved (O:{row['open']:.2f}, C:{row['close']:.2f})")
            total_inserted += 1
        except Exception as e:
            print(f"  ❌ Error saving {row['symbol']}: {e}")
            
    print("\n" + "="*70)
    print(f"✅ Hoàn tất Job Spark! Đã xử lý và lưu: {total_inserted} cây nến 1h.")
    print("="*70)
